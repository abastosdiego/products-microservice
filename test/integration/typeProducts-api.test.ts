...
describe("Type Products API test", () => {
    ...
    it("delete type product", async () => {
        const responseGet = await axios.get('http://localhost:3000/type-products');
        const id = responseGet.data?.[0]?.['id'];
        if (!(id)){
            throw new Error('Os dados recebidos são inválidos ou incompletos.');
        }
        const productsQuantity = responseGet.data.length;
        const responseDelete = await axios.delete(`http://localhost:3000/type-products/${id}`);
        expect(responseDelete.status).toBe(200);
        const responseGetAfterDelete = await axios.get('http://localhost:3000/type-products');
